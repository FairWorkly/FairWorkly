...
public class CsvValidator : ICsvValidator
{
    ...
    public Result<List<ValidatedPayrollRow>> Validate(
        List<string[]> rows,
        AwardType awardType,
    ...
    )
    {
        if (rows.Count == 0)
        {
            ...
            return Result<List<ValidatedPayrollRow>>.Of422(
            ...
            if (!string.Equals(headerRow[i].Trim(), ExpectedHeaders[i], StringComparison.Ordinal))
            {
                errors.Add(
                    new Csv422Error
                    {
                        ...
                        Field = "Header",
                        Message =
                            $"Column {i + 1}: expected \"{ExpectedHeaders[i]}\", found \"{headerRow[i].Trim()}\"",
                    }
                );
            }
        }
    ...
    private static List<Csv422Error>? ValidateGlobal(List<string[]> dataRows)
    {
        ...
        var payPeriodInconsistent = dataRows.Any(r =>
            r[3].Trim() != firstStart || r[4].Trim() != firstEnd
        );
    ...
    )
    {
        ...
        for (var i = 0; i < dataRows.Count; i++)
        {
            ...
            {
                errors.Add(
                    CreateError(rowNumber, "Hourly Rate", "Hourly Rate must be a positive number")
                );
                rowHasError = true;
            }
            else
            ...
            {
                errors.Add(
                    CreateError(rowNumber, "Ordinary Hours", "Ordinary Hours must be a number >= 0")
                );
                rowHasError = true;
            }
            else
            ...
            )
            {
                errors.Add(
                    CreateError(
                        rowNumber,
                        "Superannuation Paid",
                        "Superannuation Paid must be a number"
                    )
                );
                rowHasError = true;
            }
    ...
    private static void ValidateOptionalHoursPay(
        ...
        string label,
        int rowNumber,
        ValidatedPayrollRow validatedRow,
        List<Csv422Error> errors,
    ...
    )
    {
        ...
        if (!string.IsNullOrEmpty(hoursStr))
        {
            ...
            {
                errors.Add(
                    CreateError(rowNumber, $"{label} Hours", $"{label} Hours must be a number >= 0")
                );
                rowHasError = true;
    ...
    private static DateTimeOffset ParseDateToUtc(string dateStr) =>
        new(
            DateOnly
                .ParseExact(dateStr, "yyyy-MM-dd", CultureInfo.InvariantCulture)
